...
// Returns true if MIDIType input can have a channel. Needs modifying if
// capability for more channel type messages is added to library
...
static uint8_t parse(void) {
  ...
  if (d.mPendingMessageIndex == 0) {
    ...
    if (isChannelMessage(getTypeFromStatusByte(d.mRunningStatus))) {
      // If the byte is < 0x80 then it cant be a status byte so it must be
    ...
    // NOTE: Put code for dealing with 1 byte messages here if using 1 byte
    // messages
